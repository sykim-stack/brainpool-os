...
- **나이테(growth ring)**: House = 시간이 쌓이는 공간의 시그니처. 프로필사진을 감싸는 프레임. 기본값은 House 정적 데이터(생성일 경과, 활동량 등)로 계산 —
        CoreNull 단독으로 지금 바로 의미 있게 동작
        CoreHub가 붙으면 그 계산에 추천/개인화 가중치가 추가로 얹힘 (Phase B) §5 표.
